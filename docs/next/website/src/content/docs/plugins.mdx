...
full Herdr client frame. The popup is centered across the complete frame and
dims the sidebar, tab bar, and tiled panes behind it. It receives all terminal
input, including Escape, and closes when the command exits or a `popup.close`
request is sent. Dimensions smaller than the popup minimum are clamped.
