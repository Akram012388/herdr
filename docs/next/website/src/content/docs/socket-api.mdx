...
percentages such as `"80%"` of the full Herdr client frame. Omitted dimensions
default to half the frame. Popups are centered across the complete frame, dim
the interface behind them, and clamp too-small values to the popup minimum. A
popup has no pane ID, remains outside all `pane.*` and agent APIs, emits no pane
lifecycle events, leaves plugin focus context on the underlying tiled pane, and
does not export `HERDR_PANE_ID` to its process. Popup launch returns `ok`;
`popup.close` closes the active popup and returns
