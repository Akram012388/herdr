...
cells or percentages such as `80%` of the full Herdr client frame; omitted
dimensions default to half the frame, and values smaller than the popup minimum
are clamped. Popups center across the complete frame and dim the interface
behind them. A popup is not a Herdr pane, does not export `HERDR_PANE_ID`, and
does not participate in pane or agent APIs. Native non-terminal plugin panes
are a later surface.
