...
percentage of the full Herdr client frame. The popup is centered across the
complete frame and dims the interface behind it. Dimensions include the popup
border, and values smaller than the popup minimum are clamped. Popup commands
do not receive `HERDR_PANE_ID`; use `HERDR_ACTIVE_PANE_ID` for the underlying
tiled pane.
